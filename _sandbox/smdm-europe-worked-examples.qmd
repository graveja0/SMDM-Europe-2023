...
1. Fit a mortality model and construct a simple "alive-dead" model that replicates life table data.
2. Add in a CVD sick state and replicate overall life-expectancy.
3. Add in a cause-specific (CVD) death state using cause-deleted life tables and cause-specific mortality rate. Verify that it replicates overall life expectancy.
4. Using the "Healthy-CVD-Dead" natural history model, count up the total number who become sick. Show how standard conversion formulas will miscount the total number who become ill.

## Afternoon: "Top Down" 

5. Start with the HIV model from the Briggs textbook. Solve for the continuous generator matrix.
6. Fit a mortality model to German life table data and swap in an age-specific death rate into the HIV monotherapy model. How do modeled costs and QALYs differ?
7. Suppose there is a new combination therapy with a hazard ratio and 95% confidence interval. Incorporate this into the HIV model and calculate ICERs. 
8. Now solve for the PSA distribution parameters for this new hazard ratio. 
9. Use copulas to simulate correlated direct medical care and community care costs. 
...
- [All] Do we need a full-fledged CEA for the CVD model? Or would just creating the natural history model suffice?
...
![](media/digraph-healthy-sick-dead.png){fig-align="center" height="300px"}
...
![](media/digraph-healthy-sick-dead-HR.png){fig-align="center" height="300px"}
...
![](media/digraph-healthy-sick-dead-cause-deleted.png){fig-align="center" height="300px"}
