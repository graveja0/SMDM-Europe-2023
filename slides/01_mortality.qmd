---
title: "1. Dealing with Mortality"
editor: source
format:
  revealjs:
    transition: fade
...
[^1]: Note that we divide by the initial cohort size (`r params$n_cohort`) to normalize the results to a single individual for life expectancy calculations.
...
laws <- 
    ...
    mutate(MODEL = ifelse(CODE == "invweibull","$\\mu[x] = 1/\\sigma (x/M)^{[-M/\\sigma - 1]} / [e^{(x/M)^{(-M/\\sigma)}} - 1]$",MODEL)) %>% 
    ...
    mutate(MODEL = ifelse(CODE == "invgompertz","$\\mu[x] = [1-e^{(M-x)/\\sigma}]/[e^{(M-x)/\\sigma}-1]$",MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "gompertz", "$\\mu[x] = A e^{Bx}$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "gompertz0", "$\\mu[x] = 1/\\sigma e^{(x-M)/\\sigma}$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "makeham", "$\\mu[x] = A e^{Bx} + C$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "makeham0", "$\\mu[x] = 1/\\sigma e^{(x-M)/\\sigma} + C$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "strehler_mildvan", "$\\mu[x] = K e^{-V_0 (1-Bx)/D}$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "vandermaen", "$\\mu[x] = A+Bx+Cx^2 + I/[N-x]$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "beard", "$\\mu[x] = e^{Bx}/[1+KA e^{Bx}]$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "beard_makeham", "$\\mu[x] = A e^{Bx}/[1 + KA e^{Bx}]$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "ggompertz", "$\\mu[x] = A e^{Bx}/(1+AG/B[e^{Bx}-1])$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "vandermaen2", "$\\mu[x] = A + Bx + I/[N-x]$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "quadratic", "$\\mu[x] = A + Bx + Cx^2$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "kannisto", "$\\mu[x] = A e^{Bx}/[1+A e^{Bx}]$", MODEL)) %>%
    mutate(MODEL = ifelse(CODE == "kannisto_makeham", "$\\mu[x] = A e^{Bx}/[1+A e^{Bx}]+C$", MODEL)) %>%
