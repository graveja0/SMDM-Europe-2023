...
- For `approxfun()`:
  - `x` is quantile in CDF for survival, i.e., 1 - Survival^[Recall that survival is $\exp(-H)$, where $H$ is the cumulative hazard]
  - `y` is the age. 
...
params = 
  list(
    t_names = c("natural_history","foo"),       # Strategy names. 
