...
::: nonincremental
- Suppose the interquartile range for a key cost variable is [300,750]
- We may have obtained this from the literature, from tabulating cost data, or from collating expert opinion.
:::
...
```{r}
#| echo: true
qlnorm(0.25, mean = mu, sd = sigma)
qlnorm(0.75, mean = mu, sd = sigma)
```
...
```{r}
#| echo: true
qgamma(0.25,shape = alpha_, scale = beta_)
qgamma(0.75,shape = alpha_, scale = beta_)
```
