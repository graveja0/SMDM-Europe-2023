...
-  We have seen issues with probability scale estimators.
-  This becomes numerical unstable in the tails.
-  Optimizer will search the tails.
-  While analytically correct, is not numerically stable.
:::
...
- Log scale has better stable and works for a much wider range.
- Tweaking parameters to converge still happens.
:::

$$\min_{x_1, x_2 \in \mathcal{R}}\,\, (\log(F(x_1))-\log(p_1))^2+(\log(F(x_2))-\log(p_2))^2$$
...
library(pracma)
norm <- function(x1, p1, x2, p2, mu, sigma)
 (pnorm(x1, mu, sigma, log.p=TRUE)-log(p1))^2 +
 (pnorm(x2, mu, sigma, log.p=TRUE)-log(p2))^2
