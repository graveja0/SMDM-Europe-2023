...
subtitle: "Rates, Probabilities, and Embedding"
editor: source
format:
  revealjs:
    transition: fade
...
-   Transitions are goverened by a single rate from Healthy {{< fa arrow-right >}} CVD
...
-   Compound transition: Healthy {{< fa arrow-right >}} CVD {{< fa arrow-right >}} CVD Death in a single cycle.
...
-   To accurately capture the underlying continuous time process, there should be a transition probability defined for a seemingly impossible transition: Healthy {{< fa arrow-right >}} CVD Death.
...
digraph G {
    ...
    Healthy -> Healthy ;
    CVD -> CVD;
    Test [pos="1,.65!" label = "" fontcolor="white" shape="point" color="red"];
    Test2 [pos="3,-.9!" label = "" fontcolor="white" shape=none ];
    Healthy -> Test [color = "red" style=dashed arrowhead=none];
    Test -> CVDDeath [color = "red" style=dashed];
    Healthy -> CVD [label="r_H_CVD = 0.15"]; 
    CVD -> CVDDeath [label="  hr_CVD * r_HD\nhr_CVD=10" ];
    CVDDeath -> CVDDeath;
    Healthy -> Dead [label="r_HD=0.01"];
...
digraph G {
    ...
    Healthy -> Healthy ;
    CVD -> CVD;
    Healthy -> CVDDeath [color = "red" style=dashed];
    Healthy -> CVD [label="r_H_CVD = 0.15"]; 
    CVD -> CVDDeath [label="  hr_CVD * r_HD\nhr_CVD=10" ];
    CVDDeath -> CVDDeath;
    Healthy -> Dead [label="r_HD=0.01"];
...
3.  Individual dies from background causes before they would have developed CVD in the same cycle.
...
-   As constructed, we have ruled out the possibility of compound transitions (Healthy {{< fa arrow-right >}} CVD {{< fa arrow-right >}} CVDDeath) in a single cycle.
...
-   Existing publications' decision thresholds are generally okay if they misspecified rates.
...
## What Is Commonly Taught {auto-animate="true"}
...
## What We'll Teach You {auto-animate="true"}
...
![](images/digraph-healthy-sick-dead-cause-deleted.png){fig-align="center" height="300px"}
