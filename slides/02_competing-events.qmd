...
::: {.column width="60%"}
```{r}
tmp_ <- unname(params_mort2_10y$mP)
dimnames(tmp_) <- list(params_mort2_10y$s_names,params_mort2_10y$s_names)
round(params_mort2_10y$mP,3)
```
:::

::: {.column width="40%"}

- This matrix "captures" all transitions from Healthy {{< fa arrow-right >}} CVD in a single transition probability `r round(params_mort2_10y$mP[1,2,1],3)`. 
...
- In doing so, our model no longer captures a continuous-time disease progression process. 
...
params_mort_10y_emb <- modifyList(params_mort,list(cycle = 10, n_cycles=10))

params_mort_10y_emb$mR <- 
  with(params_mort_10y_emb,{
...
params_mort_10y_emb$mP <- lapply(params_mort_10y_emb$mR,expm)

round(params_mort_10y_emb$mP[[1]],3)
