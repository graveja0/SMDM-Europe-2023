...
We can similarly use the same function as defined in @sec-b3 to embed the transition probability matrix. 
...
Again we can use the function defined in @sec-b4 to construct the trace for each strategy:
...
@fig-compare plots state occupancy by cycle for strategy A (conversion) vs. strategy B (solving for the generator matrix). As the figure shows, the two strategies yield broadly similar results, but there are slight differences in state occupancy. 
