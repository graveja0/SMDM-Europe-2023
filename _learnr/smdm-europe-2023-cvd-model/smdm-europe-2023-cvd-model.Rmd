...
#   # Convert to annual time step
#   mutate(m =  -log(1-q)/(1/12)) %>% 
#   mutate(q = 1 - exp(-m)) %>% 
#   mutate(t = min_age + (row_number()-1)/12)
# 
# tr_year %>% 
#   as_tibble() %>% 
#   mutate(lx = radix * alive) %>% 
...
#   ggplot(aes(x = t, y = log(q))) + geom_step() + 
#   geom_point(data = lt %>% filter(age>=min_age & age <max_age), aes(x = x, y = log(qx)),colour="red")+
#   hrbrthemes::theme_ipsum_pub(base_family = "Arial") + 
#   geom_step(data = tmp, colour = "blue", pch =19)
# 
...
```{r 2a, exercise = TRUE, exercise.eval = TRUE, exercise.setup = "setup2"}
#| exercise.lines: 30
...
life_exp_markov <- 
  cycle_adj(tr_year %*% params$payoff_qaly, 1)
