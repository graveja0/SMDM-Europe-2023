...
If $\epsilon_M - \epsilon_{ref} \sim 0$ the impact to the decision threshold is minimal.

-   These decision thresholds are semi-*robust* against this misspecification.
-   Restated: Single model runs are biased under misspecification, but decision threshold comparisons cancel this out.

## Rate Misspecification Conclusions

-   Existing publications decision thresholds are generally okay if they misspecified rates under 3a.
-   Check the maximum rate of an event as a rough estimate of effect.
-   Adjusting the timestep does little. The error still exists.

## Other

-   Discrete Event Simulation automatically handles competing events.
-   microsimulation is embedded in a time step and requires Markov Rate conversion.
...
::: incremental
-   Constructing our final transition probability matrix---for natural history or more specifically for for a strategy under consideration--may require further adjustment.
...
deaths.male <- read.table(here::here("_sandbox/gompertz-mortality/data/mltper_1x1.txt"),skip=2,header=T,as.is=TRUE)
deaths.female <- read.table(here::here("_sandbox/gompertz-mortality/data/fltper_1x1.txt"),skip=2,header=T,as.is=TRUE)
...
parameters <- read.csv(here::here("_sandbox/gompertz-mortality/data/gompertz-mortality.csv"))
```
